...
var creatorList = ['ArdiTech07','ArdiGans'];
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != '4rd1key') return res.json(loghandler.invalidKey)
...
/*outer.get('/music/joox', async (req, res, next) => {
	...
	if(apikeyInput != '4rd1key') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != '4rd1key')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/zeeoneofc/tes/asupan/vietnam.json`))
